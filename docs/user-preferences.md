...
- saved, watched, favourite, and rated archive signals remain the primary recommendation anchors.
...
Account export schema version 3 contains the current preference snapshot alongside the member profile, library, and synchronization status. Permanent account deletion recursively removes the `users/{uid}` tree, including the preference document, and then clears the user-scoped browser key.
